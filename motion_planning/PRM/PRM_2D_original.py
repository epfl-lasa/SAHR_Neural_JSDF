...
while 1:
    ...
    if dynamic:
        ...
        for i in range(5):  # update samples to make them closer to the safety_dis
            ...
            samples[2:static_node_num, :] = samples[2:static_node_num, :] - (dis[2:static_node_num].reshape(-1, 1) - safety_dis * 4) / grad[2:static_node_num, 1:3] * 0.01 * 3
    ...
    if np.linalg.norm(np.array(path[-1]) - np.array(s_goal)) < 1e-5:
        ...
        if optimize:
            ...
            pairs = collision.get_dis(edge_samples, gradient=False, sample=edge_sample_num_tmp, safety_dis=safety_dis / 5,
                                      real_dis=True)
            start_end_p = []
