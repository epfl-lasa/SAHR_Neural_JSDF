...
for step in range(step_max):
    ...
    inc_k = 19
    tmp = knn.predict(samples[0:2, :], k=k0 + inc_k + 1)[:, 1:, :]
    s2[0] = list(map(tuple, tmp[0, :, :]))
    s2[1] = list(map(tuple, tmp[1, :, :]))
    graph_16D = graph(dim=dim)
    graph_16D.edges = dict(zip(s1, s2))  # {v1: [a1,a2,a3], v2; [...],...}
    d_star = DStar(s_start, s_goal, graph_16D, "euclidean")

    start_p = np.repeat(samples[:2, :], repeats=k0 + inc_k, axis=0)
    start_p = np.concatenate([start_p, np.repeat(samples[2:, :], repeats=k0, axis=0)])
    # start_p = np.repeat(samples, repeats=k0, axis=0)
    edge_samples_ = np.linspace(start_p, np.vstack(s2),
                                edge_sample_num, axis=1)  # (n*k, edge_sample_num, dim)
...
print('success rate', success / step_max)
