...
g = 1
dec = 0.1
name = 'obj_single_convex_left_' + str(g) + '_combined.npy'
data = np.load('../dataset/data/' + name)
...
if g == 1:  # index
    # dis_min = -0.01
    # dis_max = 0.1
    obj_bound = np.array([[-0.04, -0.12, 0.06], [0.12, 0.02, 0.22]])
    dis_scale = np.array([[-0.009, -0.009, -0.009, -0.009, -0.013], [0.12, 0.075, 0.07, 0.12, 0.13]])

elif g == 4:  # thumb
    obj_bound = np.array([[-0.06, -0.12, -0.04], [0.14, 0.05, 0.13]])
    dis_scale = np.array([[-0.01, -0.009, -0.009, -0.009, -0.013], [0.12, 0.1, 0.08, 0.12, 0.15]])
elif g == 0:  # palm
    obj_bound = np.array([[-0.1, -0.2, -0.05], [0.1, 0.2, 0.15]])
    dis_scale = np.array([[-0.006, -0.02], [0.15, 0.15]])
...
model_name = 'single_' + str(g) + '02_left'
