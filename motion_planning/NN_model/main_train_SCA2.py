...
from nn_model import Net, Net_5, loss_SCA_neg, load_dataset_SCA, validate_SCA_neg
...
if right_hand:
    ...
    name = 'allegro_SCA_4000000_left_04.npy'
    path = '../dataset/data/'
    dec = 0.1
add_cos_sin = 1
data, num_s, dis_scale = load_dataset_SCA(name, path='../dataset/data/', add_sin_cos=add_cos_sin, normalization=3, dec=dec)
print('dis_scale', dis_scale)
...
for cv in range(cv_num):
    data_index = np.random.choice(np.arange(0, data.shape[0]), int(data.shape[0] / 2), replace=False)
    a = np.arange(0, data.shape[0])
    data_index_test = np.setdiff1d(a, data_index)
    print('train set', len(data_index), 'test set', len(data_index_test))
    ...
    resume = 0
    test_accuracy = 1
    model_name = 'model_new_01_N5_left_cv_'
    path_check_point = 'models/' + model_name + '.pt'  # modified loss, penalty output<0 and target>0
    epoch = 0
    if resume:
        ...
        print('initialize a new model, cv = ', cv)
        ...
        if epoch % 1000 == 0:
            ...
            if loss < best_loss:
                torch.save({
                    ...
                    'add_lr': np.array([add_cos_sin, learn_rate, epoch])
                }, path_check_point)
                best_loss = loss
                # save parameters
                # np.savez('models/' + model_name + '_para',
